...
namespace DoAnAdmin.Models
{
    public class HamXuLy
    {
        ...
        public string createIDAccessorys()
        {
            string prodIDLast;
            try
            {
                //Lấy mã cuối cùng
                prodIDLast = db.Products.Where(p=>p.productType_id!=1).OrderByDescending(p => p.id).Select(p => p.id).First().ToString();
                //xem la latop hay phukien
            }
            catch (Exception)
            {
                return "PKC000000";
